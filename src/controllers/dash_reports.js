...
const dashboardLastQuarter = async (req, res) => {
  ...
  array.lastQuarterPremByMonth = [
    Number(aux3.replace(/[^0-9\.]+/g, "")),
    Number(aux2.replace(/[^0-9\.]+/g, "")),
    Number(aux1.replace(/[^0-9\.]+/g, "")),
  ];
  lquarter =
    Number(aux1.replace(/[^0-9\.]+/g, "")) +
    Number(aux2.replace(/[^0-9\.]+/g, "")) +
...
const dashboardWeekReports = async (req, res) => {
  ...
  for (let j = 0; j < 7; j++) {
    ...
    sumPrem = sumPrem + aux;
    sumPol =
      sumPol + Number(result1.rows[j].policies.replace(/[^0-9\.]+/g, ""));
